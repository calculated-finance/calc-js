...
export class AccountSequenceMismatchError extends Data.TaggedError(
        ...
        Effect.gen(function* () {
          ...
          const result = yield* Effect.tryPromise({
            try: () =>
              ...
              console.log(
                `Failed to simulate transaction on chain ${chain.id}: ${error.message}`
              );
        ...
        Effect.gen(function* () {
          ...
          const result = yield* Effect.tryPromise({
            try: () =>
              ...
              console.log(
                `Failed to sign and broadcast transaction on chain ${chain.id}: ${error.message}`
              );
            ...
            Effect.catchTag(
              "AccountSequenceMismatchError",
              (error) => new TransactionSubmissionFailed({ cause: `${error}` })
            )
          );
...
export const createCosmosQueryClient = (chain: CosmosChain) =>
  Effect.gen(function* () {
      ...
      fetchBalances: (address: string) =>
        Effect.tryPromise({
          try: async () => {
            ...
            new BalancesFetchFailed({ cause: error.message }),
        }),
        ...
        Effect.gen(function* () {
          const transactions = yield* Effect.tryPromise({
            try: () =>
              ...
              new TransactionFetchFailed({ cause: error.message }),
          });
          ...
          return transactions
            .map((tx) => {
              try {
                ...
                const executeMsg = decodedTx.body.messages.find(
                  (msg) =>
    ...
    Effect.catchAll((error) =>
      Effect.fail(
        new Error(`Failed to create Cosmos query client: ${error.message}`)
      )
    )
  );
