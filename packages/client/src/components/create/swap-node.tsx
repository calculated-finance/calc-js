...
function JsonEditor<T, U>({
  ...
  return (
    <div
      className="mt-4 max-h-150 w-300 overflow-auto"
        ...
        >
              ...
              >
                <div className="flex flex-col gap-4 text-xl">
                    ...
                    children={field => (
                      <div className="flex flex-col gap-0">
                        <div
                          className={`transition-all duration-300 ease-in-out ${
                            isHelpOpen
                              ? 'px-1 pt-8 pb-2 opacity-100'
                              : 'max-h-0 overflow-hidden opacity-0'
                          } `}
                        >
                          <code className="text-sm text-pretty text-blue-400/80">
                            Enter the amount you want to swap. CALC may try to
                            swap this exact amount, or use it to calculate an
                            adjusted amount if SCALED adjustment is selected.
                          </code>
                        </div>
                        <div className="flex flex-col gap-2">
                          ...
                          >
                            <code className="ml-1 font-mono text-sm text-zinc-400">
                              swap_amount
                            </code>
                          </div>
                    ...
                    children={field => (
                      <div className="flex flex-col gap-0">
                        <div
                          className={`transition-all duration-300 ease-in-out ${
                            isHelpOpen
                              ? 'px-1 pt-4 pb-2 opacity-100'
                              : 'max-h-0 overflow-hidden opacity-0'
                          } `}
                        >
                          <code className="text-sm text-pretty text-blue-400/80">
                            Enter the minimum amount to receive each swap -
                            basically set a price floor or ceiling. Hit the
                            switch button below to invert the direction.
                          </code>
                        </div>
                        <div className="flex flex-col gap-2">
                          <div
                          ...
                          <div className="flex gap-4 rounded bg-zinc-900">
                            ...
                            >
                              <code
                                ...
                                style={{
                                  color:
                                    action.swap.minimum_receive_amount.color,
                                }}
                              >
                                {
                                  form.getFieldValue(
                        ...
                        {!field.state.meta.isValid ? (
                          ...
                          <div className="flex justify-end gap-2 pt-1">
                            <code className="text-sm text-zinc-200">
                              {isBuying ? 'buy' : 'sell'} at{' '}
                              {isBuying ? '<' : '>'} $
                              {isBuying
                                ? formatNumber(
                                    field.state.value === 0
                                      ? 0
                                      : form.state.values.swap.swap_amount
                              ...
                              {isBuying
                                ...
                                : form.state.values.swap.minimum_receive_amount
                                    .displayName}
                            </code>
                            <code
                  ...
                  <form.Field
                    name="swap.routes"
                    children={field => {
                      const hasFinRoute = field.state.value.some(
                        r => 'fin' in r
                      )
                      const canRouteOnFin = routes.some(r => 'fin' in r)
                      const hasThorchainRoute = field.state.value.some(
                        r => 'thorchain' in r
                      )
                      const canRouteOnThorchain = routes.some(
                        r => 'thorchain' in r
                      )
                      return (
                        <div className="flex flex-col gap-0">
                          <div
                            className={`transition-all duration-300 ease-in-out ${
                              isHelpOpen
                                ? 'pt mt-[-5px] px-1 pb-4 opacity-100'
                                : 'max-h-0 overflow-hidden opacity-0'
                            } `}
                          >
                            <code className="text-sm text-pretty text-blue-400/80">
                              Select the possible routes that the swap can take.
                              Not all pairs can be take all routes.
                            </code>
                          </div>
                          <div className="mt-[-12px] flex flex-col gap-2">
                            ...
                            <div className="flex h-9 w-full items-center gap-2 p-1">
                              ...
                              <code
                                ...
                                style={{
                                  backgroundColor:
                                    hasThorchainRoute && canRouteOnThorchain
                                      ? '#2CBE8C'
                                      : 'transparent',
                                  color: !canRouteOnThorchain
                                    ? 'gray'
                                    : hasThorchainRoute
                                      ? 'black'
                                      : '#2CBE8C',
                                  border: canRouteOnThorchain
                                    ? '1px solid #2CBE8C'
                                    : '1px solid gray',
                                }}
                                onClick={() => {
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="flex gap-8">
                            <form.Field
                              ...
                              children={field => (
                                <div className="flex flex-1 flex-col gap-0">
                                  ...
                                  >
                                    <code className="text-sm text-pretty text-blue-400/80">
                                      Enter the maximum slippage % allowed for
                                      each swap.
                                    </code>
                                  </div>
                                  <div className="flex flex-col gap-2">
                            ...
                            <div className="flex flex-2 flex-col gap-0">
                              ...
                              >
                                <code className="text-sm text-pretty text-blue-400/80">
                                  Select the adjustment type for the swap. FIXED
                                  will swap the exact amount, while SCALED will
                                  adjust the amount based on the price of the
                                  asset.
                                </code>
                              </div>
                              <div
                              ...
                              >
                                ...
                                <div className="flex h-full w-full items-center justify-around">
                                  ...
                                  <code className="text-xl text-zinc-400">
                                    |
                                  </code>
                                  <code
                          ...
                          <form.Field
                            name="swap.maximum_slippage_bps"
                            children={_ =>
                              !form.state.fieldMeta['swap.maximum_slippage_bps']
                                ?.isValid && (
                                <p className="font-mono text-sm text-red-500/60">
                                  {form.state.fieldMeta[
                                    'swap.maximum_slippage_bps'
                                  ]?.errors.join(', ')}
                                </p>
                              )
                            }
                              ...
                              children={field => (
                                <div className="flex flex-col gap-0">
                                  <div
                                    className={`transition-all duration-300 ease-in-out ${
                                      isHelpOpen
                                        ? 'px-1 pb-2 opacity-100'
                                        : 'max-h-0 overflow-hidden opacity-0'
                                    } `}
                                  >
                                    <code className="text-sm text-pretty text-blue-400/80">
                                      Enter the scalar value for the swap
                                      adjustment. This will adjust the amount
                                      swapped based on the price of the asset.
                                      You can see some indication of swap
                                  ...
                                  <div className="flex w-full gap-8">
                                    ...
                                    <div className="flex w-full flex-2 flex-col gap-2">
                                      ...
                                      >
                                        <code className="ml-1 font-mono text-sm text-zinc-400 opacity-0">
                                          gradient
                                        </code>
                                      </div>
          ...
          >
            ...
            {isEditingJson && (
              <JsonEditor
                data={Effect.runSync(
                  Schema.encode(Swap)(form.state.values.swap)
                )}
                schema={Swap}
                onSave={() => {
