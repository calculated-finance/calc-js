...
/**
 * The full swap editing form, shared by the SWAP action node and the
 * CHECK LIQUIDITY (can_swap) condition node — both edit the same Swap
 * payload. `footer` renders inside the form below the fields (e.g. the
 * add-next-step picker).
 */
export function SwapEditor({
  ...
  return (
    ...
    >
      ...
      <div
        className={`transition-all duration-300 ease-in-out ${
          isSelectingAnyAsset ? "h-auto opacity-100" : "h-0 overflow-hidden opacity-0"
        }`}
      >
