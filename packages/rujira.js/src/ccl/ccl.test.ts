...
import { CclQuadratic } from "./quadratic";
import { CclLinear } from "./linear";
import { Ccl } from "./base";
import { createCcl, generateCclDistribution } from "./ccl";
import type { CclModel, CclRangeConfig } from "./types";
...
describe.each(models)("balanceRatio roundtrip [$name]", ({ create }) => {
  ...
  it.each(sigmas.flatMap((s) => prices.map((p) => ({ s, p }))))(
    "σ=$s p=$p → quoted price matches",
    ({ s, p }) => {
...
describe.each(models.map((m) => m.name))("generateCclDistribution [%s]", (model) => {
  ...
  it("avg fill prices stay in [low, high] when price > high", () => {
    const dist = generateCclDistribution({ ...baseConfig, price: 5 });
    expect(dist.avgBidFillPrice).toBeGreaterThanOrEqual(baseConfig.low);
    expect(dist.avgBidFillPrice).toBeLessThanOrEqual(baseConfig.high);
    expect(dist.avgAskFillPrice).toBe(0);
  });

  it("avg fill prices stay in [low, high] when price < low", () => {
    const dist = generateCclDistribution({ ...baseConfig, price: 2 });
    expect(dist.avgAskFillPrice).toBeGreaterThanOrEqual(baseConfig.low);
    expect(dist.avgAskFillPrice).toBeLessThanOrEqual(baseConfig.high);
    expect(dist.avgBidFillPrice).toBe(0);
  });

  it("askPrice/bidPrice are clamped to [low, high]", () => {
    const above = generateCclDistribution({ ...baseConfig, price: 5 });
    expect(above.askPrice).toBeLessThanOrEqual(baseConfig.high);
    expect(above.bidPrice).toBeLessThanOrEqual(baseConfig.high);

    const below = generateCclDistribution({ ...baseConfig, price: 2 });
    expect(below.askPrice).toBeGreaterThanOrEqual(baseConfig.low);
    expect(below.bidPrice).toBeGreaterThanOrEqual(baseConfig.low);
  });
...
describe("distribution skew (quadratic — symmetric)", () => {
  const cfg: CclRangeConfig = {
    high: 4, low: 3, price: 3.5,
    sigma: 0, spread: 0.01, delta: 0.05, model: "quadratic",
  };
...
describe("distribution skew (linear — directional)", () => {
  const cfg: CclRangeConfig = {
    high: 4, low: 3, price: 3.5,
    sigma: 0, spread: 0.01, delta: 0.05, model: "linear",
  };
